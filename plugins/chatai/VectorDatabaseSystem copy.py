...
class RAGManager:
    ...
    async def _get_last_single_index(self) -> int:
        """获取最近的单条消息的索引。"""
        # 找到最近的单条消息
        result = await self.olromadb.get_top_records(self.group_collection, 1)
        metadatas = result["metadatas"]
        if metadatas:
            metadata = metadatas[0]
            if "type" in metadata: # 如果最近的记录有type字段
                if metadata["type"] == "single": # 如果上一个记录的类型为单条消息
                    last_index = metadata["index"]
                    if not isinstance(last_index, int):
                        raise MetadataFormatError("最近的单条消息的index字段值不是整数")
                elif metadata["type"] == "chunk": # 如果上一个记录的类型为消息块
                    last_index = 0
                else:
                    raise MetadataFormatError("最近的记录的type字段值不是'single'或'chunk'")
            else:
                raise MetadataFormatError("最近的记录没有type字段")
        elif self.olromadb.count_collection_records(self.group_collection) == 0:
            last_index = 0
        else:
            raise DatabaseConsistencyError("没有找到最近的单条消息")
        return last_index

    async def _get_last_chunk_index(self, single_number: int) -> int:
        """获取最近的块消息的索引。

        根据已存在的单条消息的数量，找到最近的块消息的索引。

        Args:
            single_number (int): 已存在的单条消息的数量。
        """
        # 找到最近的块消息
        result = await self.olromadb.get_top_records(self.group_collection, single_number + 1) # 获取最近的single_number + 1条记录
        if len(result["ids"]) < single_number + 1:
            if len(result["ids"]) == single_number: # 只有single_number条记录
                return 0
            else:
                raise DatabaseConsistencyError("找到的记录的数量小于单条消息数量")
        elif len(result["ids"]) > single_number + 1:
            raise DatabaseConsistencyError("找到的记录的数量大于单条消息+1的数量")
        else:
            metadatas = result["metadatas"]
            if not metadatas:
                raise DatabaseConsistencyError("没有找到最近记录的元数据")
            # 提取最近的块消息的元数据
            metadata = metadatas[single_number]
            if not metadata:
                raise MetadataFormatError("最近消息块的元数据为空")
            if "type" not in metadata:
                raise MetadataFormatError("最近的块消息的元数据没有type字段")
            if metadata["type"] != "chunk":
                raise MetadataFormatError("预期块消息的type字段值不是'chunk'")
            if "chunk_index" not in metadata:
                raise MetadataFormatError("最近的块消息的元数据没有chunk_index字段")
            last_index = metadata["chunk_index"]
            if not isinstance(last_index, int):
                raise MetadataFormatError("最近的块消息的chunk_index字段值不是整数")
            return last_index
    ...
    async def add_message(self, message:GroupMessage|PrivateMessage):
        ...
        if index >= MAX_SINGLE_NUMBER:
            related_user_id = set() # 发言成员列表
            related_msg_id = [] # 消息列表
            earliest_send_time = 0.0 # 最早发言时间
            latest_send_time = 0.0 # 最晚发言时间
            meg_contents = [] # 消息内容列表
            singles = await self.olromadb.get_top_records(self.group_collection, MAX_SINGLE_NUMBER)
            if not singles["metadatas"]:
                raise DatabaseConsistencyError("没有找到单条消息")
            for metadata in singles["metadatas"]:
                # 检查记录格式
                if metadata["type"] != "single":
                    raise MetadataFormatError("数据库元数据错误: 前MAX_SINGLE_NUMBER条记录中存在非单条消息. 可能是单条消息的index字段值不连续")
                if not (
                    metadata['earliest_send_time'] and 
                    metadata['latest_send_time'] and 
                    metadata['message_count'] and 
                    metadata['related_user_id'] and
                    metadata['related_msg_id']
                ):
                    raise MetadataFormatError("数据库元数据错误: 前MAX_SINGLE_NUMBER条记录中存在格式不完整的单条消息")
                # 记录发言成员
                related_user_id.add(metadata["related_user_id"])
                # 记录消息ID
                related_msg_id.append(metadata["related_msg_id"])
            # 记录消息内容
            if not singles["documents"]:
                raise DatabaseConsistencyError("没有找到单条消息内容")
            for document in singles["documents"]:
                meg_contents.append(document)
            # 记录最早发言
            earliest_send_time = singles["metadatas"][0]["earliest_send_time"]
            # 记录最晚发言
            latest_send_time = singles["metadatas"][-1]["latest_send_time"]
            # 记录数据库ID
            ids = singles["ids"]
            if len(ids) != MAX_SINGLE_NUMBER or len(meg_contents) != MAX_SINGLE_NUMBER:
                raise DatabaseConsistencyError("单条消息数量不等于MAX_SINGLE_NUMBER")



            # 构建消息块元数据
            ## 获取上一个块的索引
            last_index = await self._get_last_chunk_index(MAX_SINGLE_NUMBER)

            metadata = {
                "type": "chunk",
                "chunk_index": last_index + 1,
                "related_user_id": json.dumps(list(related_user_id)), # chromadb不允许元数据的值为列表, 使用字符串代替
                "related_msg_id": json.dumps(related_msg_id),
                "earliest_send_time": earliest_send_time,
                "latest_send_time": latest_send_time,
                "message_count": len(meg_contents),
                "group_id": 
            }


             
            # 删除原来的MAX_SINGLE_NUMBER条单条消息
            await self.olromadb.delete_records_from_collection(self.group_collection, ids)
